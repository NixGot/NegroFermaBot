...
def formatedNum(num):
    number = str(num)
    return f'{float(number):,}'.replace(',', ' ')

def create_profile(id, name):
    conn = sqlite3.connect(r'profiles_data.db', check_same_thread=False)
    cur = conn.cursor()
    data = (id, name, 300, 0, 1, 0, 0, 0, 0, 0, 0, 0)
    cur.execute("INSERT INTO users VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", data)
    conn.commit()
    cur.close()
    conn.close()
    open(str(Path("individual_profile_stat", f"{id}.json")), 'a', encoding='utf-8')
    with open(str(Path("individual_profile_stat", f"{id}.json")), 'r', encoding='utf-8') as file:
        try:
            data = json.load(file)
        except json.decoder.JSONDecodeError:
            data = {'works':{}, 'BizzareFarm': {}}
    with open(str(Path("individual_profile_stat", f"{id}.json")), 'w', encoding='utf-8') as file:
...
def moneyGet(id):
    if getDB(id, 'timeout') <= int(round(time.time())):
        ...
        putDB(id, 'totalMoney', (getDB(id, 'totalMoney') + money))  
